...
export class ConsejeriaEditComponent implements OnInit , OnDestroy {
  subscripciones = [];
  ...
  ngOnInit() {
    this.subscripciones.push(this.stateService.usuaries$.subscribe(usuaries => {
      this.usuaries = usuaries;
      if (this.usuaries.length == 0) {
        this.subscripciones.push(this.usuarieService.getAll2().subscribe(usuaries => {
          this.usuaries = usuaries;
          this.stateService.setUsuaries(usuaries)
          this.setGetDatosConsejeria();
        }));
      }else{
        this.setGetDatosConsejeria();
      }
  ...
  setGetDatosConsejeria(){
    this.id = this.activatedRoute.snapshot.paramMap.get('id');
    /*cuando me llega la data la asigno al consejeria */
    if(this.id != '' && this.id != 'new'){
      this.subscripciones.push(this.consejeriasData.getById(this.id).subscribe(consejeria => {
  ...
  guardar(form: any) {
    if(this.consejeria._id != ''){
        ...
        this.subscripciones.push(this.consejeriasData.update(this.consejeria).subscribe((_) => this.router.navigate(['consejerias'])));
        ...
        if(this.usuaria){
          ...
          this.consejeria.usuarie1Id = this.usuaries.find((u) => u.id == this.usuarie1Id);
          this.consejeria.usuarie2Id = this.usuaries.find((u) => u.id == this.usuarie2Id);
          this.subscripciones.push(this.consejeriasData.insert(this.consejeria).subscribe((_) => this.router.navigate(['consejerias'])));
        }else{
